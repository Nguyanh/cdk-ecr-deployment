...
| <code><a href="#cdk-ecr-deployment.ECRDeploymentProps.property.copyImageIndex">copyImageIndex</a></code> | <code>boolean</code> | Whether the source docker image is an image index (multi-arch manifest). |
...
Whether the source docker image is an image index (multi-arch manifest).

When true, copies the image index and all underlying architecture-specific
images in a single operation. The imageArch parameter is ignored.
...
Maps architecture names to their respective tags. This makes individual
architectures discoverable by human-readable tags in addition to the
image index tag. Ignored if copyImageIndex is false.

*Example*

```typescript
const tags = { 'arm64': 'image-arm64', 'amd64': 'image-amd64' };
```
